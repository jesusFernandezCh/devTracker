    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center-modal p-4" style="background-color: rgba(0,0,0,0.4);" (click)="cerrar.emit()">
      <div class="modal-enter rounded-xl shadow-xl w-full max-w-lg border overflow-hidden"
        ...
        <div class="flex items-start justify-between gap-4 px-6 py-3 border-b" style="border-color: var(--color-gray-100);">
          <div class="min-w-0">
            <h3 class="text-lg font-semibold leading-tight" style="color: var(--color-gray-900);">Canales / Áreas</h3>
            <p class="text-sm mt-0.5 truncate" style="color: var(--color-gray-500);">{{ canalAreas().length }} canal{{ canalAreas().length !== 1 ? 'es' : '' }} registrado{{ canalAreas().length !== 1 ? 's' : '' }}</p>
          </div>
        ...
        <div class="px-6 py-4">
          ...
          @if (canalAreas().length === 0) {
            <p class="text-sm py-8 text-center" style="color: var(--color-gray-400);">No hay canales/áreas registrados.</p>
          } @else {
            <div class="custom-scrollbar max-h-[176px] overflow-y-auto -mr-2 pr-2 space-y-1">
              @for (canal of canalAreas(); track canal.id) {
                <div class="flex items-center gap-2 p-2 rounded-lg transition-colors"
                     [style.background-color]="eliminarId() === canal.id ? 'var(--color-rose-50)' : 'transparent'">
                  @if (editandoId() === canal.id) {
                    <input [value]="editandoNombre()" (input)="editandoNombre.set($any($event.target).value)" (keydown.enter)="guardarEdicion(canal.id)"
                           type="text" autocomplete="off" placeholder="Nombre del canal/área"
                    ...
                    <button (click)="guardarEdicion(canal.id)"
                    ...
                    <span class="flex-1 min-w-0 text-sm font-medium truncate" style="color: var(--color-gray-900);">{{ canal.nombre }}</span>
                    <button *appPermiso="'editar'; recurso: 'proyectos'" (click)="editar(canal.id)"
                            ...
                            [attr.aria-label]="'Editar ' + canal.nombre">
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <button *appPermiso="'editar'; recurso: 'proyectos'" (click)="confirmarEliminar(canal.id)"
                            ...
                            [attr.aria-label]="'Eliminar ' + canal.nombre">
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
export class CanalAreaModalComponent {
  ...
  protected async agregar(): Promise<void> {
    ...
    if (!nombre) {
      await this.notificacionService.notificar({tipo: 'error', descripcion: `El nombre del canal/área no puede estar vacío.`});
      return;
    }
    if (this.canalAreas().some((c) => c.nombre.trim().toLowerCase() === nombre.toLowerCase())) {
      await this.notificacionService.notificar({tipo: 'error', descripcion: `Ya existe un canal/área llamado «${nombre}».`});
      return;
    }
    await this.canalAreaService.crear(nombre);
    this.nuevoNombre.set('');
    await this.notificacionService.notificar({tipo: 'exito', descripcion: `Canal/área «${nombre}» creado.`});
  }

  protected editar(id: string): void {
    const canal = this.canalAreas().find((c) => c.id === id);
    if (!canal) return;
    this.editandoId.set(id);
    this.editandoNombre.set(canal.nombre);
  }

  protected async guardarEdicion(id: string): Promise<void> {
    const nombre = this.editandoNombre().trim();
    const actual = this.canalAreas().find((c) => c.id === id);
    if (!actual) return;
    if (!nombre) {
      await this.notificacionService.notificar({tipo: 'error', descripcion: `El nombre del canal/área no puede estar vacío.`});
      return;
    }
    if (this.canalAreas().some((c) => c.id !== id && c.nombre.trim().toLowerCase() === nombre.toLowerCase())) {
      await this.notificacionService.notificar({tipo: 'error', descripcion: `Ya existe un canal/área llamado «${nombre}».`});
      return;
    }
    await this.canalAreaService.renombrar(id, nombre);
    this.editandoId.set(null);
    this.editandoNombre.set('');
    await this.notificacionService.notificar({tipo: 'exito', descripcion: `Canal/área «${actual.nombre}» renombrado a «${nombre}».`});
  }
  ...
  protected async ejecutarEliminar(): Promise<void> {
    ...
    if (!id) return;
    const canal = this.canalAreas().find((c) => c.id === id);
    const resultado = await this.canalAreaService.eliminar(id);
    this.eliminarId.set(null);
    if (resultado === 'en-uso') {
      await this.notificacionService.notificar({
        tipo: 'alerta',
        descripcion: `No se puede eliminar «${canal?.nombre ?? ''}»: hay proyectos que lo utilizan.`,
      });
      return;
    }
    await this.notificacionService.notificar({tipo: 'exito', descripcion: `Canal/área «${canal?.nombre ?? ''}» eliminado.`});
  }
