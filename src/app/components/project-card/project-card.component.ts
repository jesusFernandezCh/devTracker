    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
      <div class="p-4">
        ...
        @if (proyecto().descripcion) {
          <p class="text-xs text-gray-400 mb-3 line-clamp-2">{{ proyecto().descripcion }}</p>
        }

        @if (proyecto().documentacion) {
          <a [href]="proyecto().documentacion" target="_blank" rel="noopener"
             class="inline-flex items-center gap-1.5 text-xs text-indigo-500 hover:text-indigo-700 transition-colors mb-3">
